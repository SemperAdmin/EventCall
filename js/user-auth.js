...
class UserAuth {
    ...
    handleWelcomeLogin(e, overlay, resolve) {
        ...
        setTimeout(() => {
            ...
            showToast(`🎖️ Welcome to EventCall, ${user.name}!`, 'success');
    ...
    /**
     * Prompt user for email login (fallback)
     */
    async promptUserLogin() {
        return new Promise((resolve) => {
            this.showLoginModal(resolve);
        });
    }

    /**
     * Show login modal (fallback for returning users)
     * @param {Function} resolve - Promise resolve function
     */
    showLoginModal(resolve) {
        // ... keep existing login modal code for fallback cases
        // This is used when the welcome page isn't needed
    }
    ...
    getUserStats() {
        ...
        const userEvents = this.filterUserEvents(events);
...
window.userAuth = userAuth;return new Promise((resolve) => {
            this.showLoginModal(resolve);
        });
    }
    ...
    showLoginModal(resolve) {
        ...
        modal.innerHTML = `
            <div class="auth-modal-content">
                <div class="auth-header">
                    <h2>🎖️ Welcome to EventCall</h2>
                    <p>Professional Military Event Management</p>
                </div>
                
                <div class="auth-form">
                    <h3>Identify Yourself</h3>
                    <p style="margin-bottom: 1.5rem; color: #6b7280; font-size: 0.875rem;">
                        Enter your email to manage your events. You'll only see events you create.
                    </p>
                    
                    <form id="auth-form">
                        <div class="form-group">
                            <label for="user-email">Military/Work Email Address</label>
                            <input type="email" id="user-email" required 
                                   placeholder="your.name@military.mil" 
                                   autocomplete="email">
                            <small style="color: #6b7280;">This identifies your events and won't be shared</small>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="user-name" 
                                   placeholder="Rank FirstName LastName"
                                   autocomplete="name">
                            <small style="color: #6b7280;">How your name appears on events you create</small>
                        </div>
                        ...
                        <button type="submit" class="btn auth-btn">🚀 Access EventCall</button>
                    </form>
                </div>
                
                <div class="auth-footer">
                    <div style="font-size: 0.75rem; color: #94a3b8;">
                        Your email is stored locally and used only for event filtering.<br>
                        No passwords required • No account creation • Complete privacy
                    </div>
                </div>
            </div>
        `;
        ...
        document.getElementById('auth-form').addEventListener('submit', (e) => {
            e.preventDefault();
            this.handleLogin(e, modal, resolve);
        });
    ...
    handleLogin(e, modal, resolve) {
        ...
        // Show welcome message
        showToast(`🎖️ Welcome, ${user.name}!`, 'success');
    ...
    showUserProfile() {
        ...
        modal.innerHTML = `
            <div class="auth-modal-content">
                <div class="auth-header">
                    <h2>👤 User Profile</h2>
                    <p>Manage your EventCall account</p>
                </div>
                ...
                <div class="auth-form">
                    <div style="background: var(--gray-50); padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem;">
                        ...
                        <strong>👤 Display Name:</strong> ${this.currentUser.name}<br>
                        <strong>🕐 Login Time:</strong> ${new Date(this.currentUser.loginTime).toLocaleString()}
                    </div>
                    
                    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                        <button class="btn btn-danger" onclick="userAuth.logout()">🚪 Logout</button>
                        <button class="btn btn-secondary" onclick="this.closest('.auth-modal').remove()">❌ Close</button>
                    </div>
                </div>
            </div>
        `;
