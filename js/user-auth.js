...
const userAuth = {
    ...
    async init() {
        ...
        try {
            const response = await fetch(window.BackendAPI.getApiUrl('/api/auth/me'));
            if (response.ok) {
    ...
    showAdminInterface() {
        ...
        // Show the admin dashboard button
    ...
    async handleLogin(event) {
        ...
        try {
            ...
            if (response.success) {
                const meResponse = await fetch(window.BackendAPI.getApiUrl('/api/auth/me'));
                if (!meResponse || !meResponse.ok) {
                    throw new Error('Failed to fetch user data after login');
                }
                const { user } = await meResponse.json();
                this.currentUser = user; // Set currentUser immediately
